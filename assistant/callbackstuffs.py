...
# --------------------------------------------------------------------#
telegraph = Telegraph()
r = telegraph.create_account(short_name="blackstorm")
auth_url = r["auth_url"]
# --------------------------------------------------------------------#
...
async def changes(okk):
    ...
    if len(changelog_str) > 1024:
        ...
        with open("blackstorm_updates.txt", "w+") as file:
            file.write(tl_chnglog)
        await okk.edit(
            get_string("upd_5"),
            file="blackstorm_updates.txt",
            buttons=Button.inline("Update Now", data="updatenow"),
        )
        remove("blackstorm_updates.txt")
        return
    await okk.edit(
        changelog_str,
        buttons=Button.inline("Update Now", data="updatenow"),
...
async def _(e):
    ...
    await e.reply(
        "`Success!\nYou are all set to use Google Drive with blackstorm Userbot.`",
