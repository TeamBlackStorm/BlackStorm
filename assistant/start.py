...
<strong>blackstorm <a href=https://github.com/TeamBlackStorm/blackstorm>[v{blackstorm_version}]</a>, powered by @BlackStormOP</strong>
"""
...
async def blackstorm(event):
    ...
    if str(event.sender_id) not in owner_and_sudos():
        ...
        if not udB.get("STARTMSG"):
            ...
            await event.reply(
                f"Hey there [{get_display_name(u)}](tg://user?id={u.id}), this is blackstorm Assistant of [{blackstorm_bot.me.first_name}](tg://user?id={blackstorm_bot.uid})!\n\n{ok}",
                buttons=[Button.inline("Info.", data="ownerinfo")],
            )
        else:
            me = f"[{blackstorm_bot.me.first_name}](tg://user?id={blackstorm_bot.uid})"
...
async def botstat(event):
    ...
    msg = """blackstorm Assistant - Stats
Total Users - {}""".format(
        ok,
    )
